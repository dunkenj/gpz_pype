...
OUTDIR = 'production/gmm{0}_n{1}_{2}_{3}'.format(GMM_THRESHOLD, NCOMP, CSL, INCSIZE)

makedir(OUTDIR)

"""
Spectroscopic Sample Setup
"""
...
unique, keep, reject = np.unique(dr14['id'], return_index=True, return_inverse=True)
dr14 = dr14[keep]

sdss_stars = vstack([dr14_south_star, dr14_north_star])
unique, keep, reject = np.unique(sdss_stars['id'], return_index=True, return_inverse=True)
sdss_stars = sdss_stars[keep]

index = np.arange(len(dr14))
i_train, i_reserve = train_test_split(index, test_size=0.33)
...
gal_x = np.array([help_specz['lupt_g']-help_specz['lupt_r'],
                  help_specz['lupt_r']-help_specz['lupt_z'],
                  help_specz['lupt_z']-help_specz['lupt_w1'],
                  help_specz['lupt_w1']-help_specz['lupt_w2'],
                  ]).T

gobs = (help_specz['nobs_g'] > 0) * (help_specz['nobs_r'] > 0) * (help_specz['nobs_z'] > 0)
gobs *= (help_specz['type'] == 'REX')
gal_x = gal_x[gobs,:]

good = ((np.isinf(gal_x).sum(1) + np.isnan(gal_x).sum(1)) == 0)
gal_x = gal_x[good,:]


gmm_stars = GaussianMixture(n_components=20,
                            max_iter=500).fit(str_x)
save_gmm(gmm_stars, f'{OUTDIR}/stargal_gmm_star.pkl')
gmm_qso = GaussianMixture(n_components=20,
                          max_iter=500).fit(qso_x)
save_gmm(gmm_qso, f'{OUTDIR}/stargal_gmm_qso.pkl')

gmm_gal = GaussianMixture(n_components=20,
                          max_iter=500).fit(gal_x)
save_gmm(gmm_gal, f'{OUTDIR}/stargal_gmm_gal.pkl')
...
for morph in ['PSF ']:
    ...
    ls_qso_d = gmm_qso.score_samples(ls_str)
    ls_gal_d = gmm_gal.score_samples(ls_str)

    star_prob = np.exp(ls_star_d) / (np.exp(ls_star_d) + np.exp(ls_gal_d) + np.exp(ls_qso_d))
    star_prob_a = np.exp(ls_star_d) / (np.exp(ls_star_d) + np.exp(ls_qso_d))

    parallax_snr = np.abs(ls_pop['parallax'] * np.sqrt(ls_pop['parallax_ivar']))
    likely_star = (ls_pop['type'][good] == 'PSF') * np.logical_or(parallax_snr[good] > 3, star_prob > 0.5)
    ...
    subsample *= (help_specz['maskbits'] == 0)    #subsample *= (help_specz['dec'] > 32.75)
    subcat = help_specz[np.where(subsample)[0]]

    Cols = ['id', 'release', 'brickid', 'objid', 'brick_primary',
            'type', 'ra', 'dec',
    ...
    sz_qso_d = gmm_qso.score_samples(sz_str)

    star_prob = np.exp(sz_star_d) / (np.exp(sz_star_d) + np.exp(sz_gal_d) + np.exp(sz_qso_d))
    ...
    subsample *= (dr14['maskbits'] == 0)    #subsample *= (help_specz['dec'] > 32.75)
    ...
    subsample *= (vhzq_specz['maskbits'] == 0)    #subsample *= (help_specz['dec'] > 32.75)

    subcat3 = vhzq_specz[np.where(subsample)[0]]
    ...
    save_gmm(gmm_ls, f'{OUTDIR}/{morph.strip()}/refpop_gmm_{morph.strip()}.pkl')
    ...
    for mx in np.arange(gmm_spec.n_components):
        ...
        if do_gpvc:
            morph_params['COVARIANCE'] = 'gpvc'
            morph_params['USE_MODEL_AS_HINT'] = '1'

            param_path = '{0}/{1}/merged_sz_subset_m{2}_{3}_gpvc.param'.format(OUTDIR, morph.strip(), mx, morph.strip())
            morph_params.write(param_path)
            bash_script.write('{0} {1}\n'.format(GPZPATH, os.path.abspath(param_path)))
            all_bash.write('{0} {1}\n'.format(GPZPATH, os.path.abspath(param_path)))
...
for morph in ['DEV ', 'EXP ', 'REX ', 'COMP']:
    ...
    subcat2 = dr14[np.where(subsample)[0]]
